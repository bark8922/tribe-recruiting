...
// Silver Medalists tab (2026-08-26) — Blake, Martin, Sashka via tribe_sm=1.
...
const SM_TABS = new Set(['silver_medalists']);
...
const RecruitingDashboard = () => {
  ...
  // Silver Medalists tab — Blake, Martin, Sashka via the tribe_sm=1 cookie set
  // by /functions/api/login.ts. No ?sm=1 URL fallback on purpose: this tab is
  // restricted to three people, so a guessable query param would defeat it.
  const canSilverMedalists = (() => {
    try {
      if (typeof document !== 'undefined') {
        const c = document.cookie.split(';').map((x) => x.trim()).find((x) => x.startsWith('tribe_sm='));
        if (c && c.split('=')[1] === '1') return true;
      }
      return false;
    } catch (_) { return false; }
  ...
  if (canSilverMedalists) visibleTabs = [...visibleTabs, 'silver_medalists'];
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
        ...
        <div className="flex gap-8">
          {visibleTabs.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              {tab === 'wbr' ? 'WBR' : tab === 'mbr' ? 'MBR' : tab === 'profitability' ? 'Profitability' : tab === 'project' ? 'Project Dashboard' : tab === 'weekly' ? 'Weekly Summary' : tab === 'tth' ? 'Time to Hire' : tab === 'ts_summary' ? 'KPI - TS Summary' : tab === 'ir' ? 'Internal Recruiting' : tab === 'finder' ? 'Candidate Finder' : tab === 'silver_medalists' ? 'Silver Medalists' : 'New Project Health'}
            </button>
          ))}
        </div>
      </div>
      <div className="px-6 py-6">
        ...
        {safeActiveTab === 'silver_medalists' && canSilverMedalists && <SilverMedalistsTab data={dashboardData} />}
      </div>
    </div>
  );
};
