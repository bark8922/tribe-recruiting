...
- **Weekly CONTACTED must come from the full aggregate, not the floored list** (found 2026-07-17 reconciling Sam's 33-vs-12): the candidate list starts at positive-response, so fresh unanswered outreach has no rows and a list-derived Contacted chip undercounts vs WBR. Ship weekly contacted counts per (job, sourcer, ISO week) in the aggregate table (extend `role_pipeline_jobs` or a small `role_pipeline_weekly` output) and render the chip as e.g. "33 contacted · 12 responded so far (listed)". All other metrics reconcile from the list by definition (a screen/ATS/offer/hire date implies the row passed the floor). CONFIRMED (Blake, keep positive-response floor for the list AND always show the full contacted count).

### ATTRIBUTION MODEL — LOCKED 2026-07-17 (the crux for dashboard reconciliation)

Verified against `weekly_summary` (the live dashboard aggregation, which stores every metric twice — DIM_TYPE `ta` and `ts`). The dashboards do NOT have one number per person; they keep two by design, and they diverge hugely for anyone who splits sourcing from ownership. Real examples W25: Andrea Akovic ta=0 / ts=119 contacted (pure sourcer); Chené Elliot ta=89 / ts=19 (owns roles her sourcer fills). Same candidate legitimately sits in one person's TS and another's TA — two questions ("who did the outreach" vs "whose desk is it"), never summed. Standard ATS practice; our stack already does it (+ a third event-based attribution for MBR Contacted).

**Attribution follows context, no toggle (Blake's model, confirmed):**
- **Role view** (a specific role selected): show EVERY candidate on the role, total = role total regardless of sourcer, each card badged with its sourcer name. Reconciles to the role owner's TA contribution for that role.
- **Sourcer view** (person page, person is a sourcer): all roles they work on, but only candidates THEY sourced (`candidate_sourcer`). Reconciles to their TS line.
- **Recruiter view** (person page, person owns roles): all candidates on their owned roles combined, badged by sourcer (`job_recruiter`). Reconciles to their TA line.

**Dual-function people (own roles AND source elsewhere — e.g. Sam owns Glovo, sources on Enam): show TWO labeled subtotals, NEVER a blended sum.** e.g. "On your roles: 33" (TA) + "Your sourcing on other roles: 21" (TS). A blended 54 matches no dashboard line. For the ~90% who are purely one function, only one line is non-zero so it reads as a single clean number — effectively the toggle-as-labels, appearing only when someone genuinely does both. Board/browse can show the union (all candidates they touch); only NUMBERS obey the attribution key.

**Build task:** confirm which line the weekly **Circle** mirrors (TS vs TA) and default a person's headline to their primary function.
